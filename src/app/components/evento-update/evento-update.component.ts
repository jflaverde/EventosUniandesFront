...
export class EventoUpdateComponent implements OnInit {
  ...
  constructor(
    ...
    private eventoListServie:EventoListService,
  ...
  ngOnInit() {
    ...
    this.registerForm = new FormGroup({
      ...
      esPresencial:  new FormControl({value: this.evento.presencial ? 'True': 'False', disabled: this.edit}, Validators.required),
    });
  ...
  editar() {
    ...
    const evento = {'nombre': nombre, 'categoria':categoria, 'lugar':lugar,
      'direccion':direccion, 'fecha_inicio':fechaInicio,  'fecha_fin':fechaFin, 'presencial':presencial, 'usuario': user };
    this.eventoUpdateService.editar(evento, this.idEvento).subscribe(response => {
